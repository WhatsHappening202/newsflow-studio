...
from newsflow.ui.dialogs.new_project_dialog import (
    NewProjectDialog,
)
...
from newsflow.ui.pages.projects_page import (
    ProjectsPage,
)
...
from newsflow.ui.widgets.navigation_panel import (
    NavigationPanel,
)
...
class MainWindow(QMainWindow):
    ...
    def _create_ui(self) -> None:
        ...
        self.narration_page = WorkflowPage(
            "Narration Workspace",
            "Import narration, review audio duration, "
            "and synchronize narration with the script.",
        )
        ...
        self.videos_page = WorkflowPage(
            "Video Library",
            "Import, preview, organize, and manage "
            "video clips for the current production.",
        )

        self.storyboard_page = WorkflowPage(
            "AI Storyboard",
            "Convert the script into scenes and match "
            "each scene with appropriate visual media.",
        )

        self.timeline_page = WorkflowPage(
            "Timeline Builder",
            "Arrange narration, images, video clips, "
            "and transitions into the final production "
            "timeline.",
        )

        self.export_page = WorkflowPage(
            "Export",
            "Choose video settings and render the "
            "finished production.",
        )

        self.settings_page = WorkflowPage(
            "Settings",
            "Configure application preferences, project "
            "defaults, and channel templates.",
        )
